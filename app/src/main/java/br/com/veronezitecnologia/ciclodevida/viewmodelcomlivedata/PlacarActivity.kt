package br.com.veronezitecnologia.ciclodevida.viewmodelcomlivedata

import android.arch.lifecycle.Observer
import android.arch.lifecycle.ViewModelProviders
import android.support.v7.app.AppCompatActivity
import android.os.Bundle
import br.com.veronezitecnologia.ciclodevida.R
...
class PlacarActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        placarViewModel?.placarVisitante?.observe(this, Observer {
            tvVisitante.text = it?.toString()
        })

        placarViewModel?.placarCasa?.observe(this, Observer {
            tvCasa.text = it?.toString()
        })
